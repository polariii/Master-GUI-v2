...
P(1)=PortfolioCVaR('ProbabilityLevel',0.95,'NumAssets',94);
P(1)=P(1).setDefaultConstraints;
...
CVaR_lim=linspace(0.03,0.0927,N_T);
    ...
    for j=1:4
        ...
        for i=1:10
            ...
            toc(t2)
            P(j)=P(1).setScenarios(Scen);
            w_crash=estimateFrontierByRisk(P(j),CVaR_lim);
...
[Pstd_MV,Pret_MV]=plotFrontier(Port_MV);
figure()
plot(Pstd_MV*100,Pret_MV*100*52,'LineWidth',2)
hold 'on'
...
ylabel('Return in %')
grid 'on'
legend('Mean-Variance','Historic','Block boostrap','Multivariate T','Multivariate Normal')
...
Price=Prices2;
Max_I=1;
pf_MV=Price*w_MV(:,Max_I);
pf_hist=Price*w_crash(:,Max_I,1);
