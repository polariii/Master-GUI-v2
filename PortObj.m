classdef PortObj < handle
    ...
    methods (Access = public)
        ...
        function importData(this,prices,benchmark,dates,priceslabels,benchmarklabel)
            ...
            returns=computeReturns(this,prices,this.returnType);
            [N,M]=size(returns);
            I=[];
            for i=1:N
                j=find(returns(i,:)==0);
        ...
        function [VaR,CVaR] = computeAssetCVaR(this);
            rets=this.returns;
            sorted_returns=sort(rets);
            [N,M]=size(rets);
            VaR_index=ceil((1-this.alpha)*N);
            VaR=sorted_returns(VaR_index,:);
            CVaR=-(mean(sorted_returns(1:VaR_index-1,:)));
        end
        ...
        % Compute and visualize parametric Value at Risk
            ...
            if isempty(returns)
                ...
                CVaR=mean(sigma*norminv(1-i)+mu);
            end
